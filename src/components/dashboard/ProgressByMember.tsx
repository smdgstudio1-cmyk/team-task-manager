  ...
  return (
    <Card>
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-ink-800">Workload by team member</h3>
        {busiest && (
          <span className="rounded-full bg-clay-50 px-2.5 py-1 text-xs font-medium text-clay-600">
        ...
        {rows.map((r) => (
          <div key={r.member.id}>
            <div className="mb-1.5 flex items-center justify-between gap-3">
              <button
                onClick={() => navigate(`/team?member=${r.member.id}`)}
                className="flex min-w-0 items-center gap-2.5"
              >
                <Avatar name={r.member.name} size="sm" />
                <span className="min-w-0">
                  <span className="block truncate text-sm font-medium text-ink-800">{r.member.name}</span>
                  {r.member.title && <span className="block truncate text-xs text-ink-400">{r.member.title}</span>}
                </span>
              </button>
              <div className="flex shrink-0 items-center gap-2 text-xs text-ink-500">
                ...
                <span className="w-9 text-right font-semibold text-ink-800">{r.progress}%</span>
              </div>
            </div>
