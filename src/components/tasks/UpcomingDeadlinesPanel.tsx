...
export function UpcomingDeadlinesPanel({ tasks, onOpenTask }: { tasks: Task[]; onOpenTask: (id: string) => void }) {
  ...
  return (
    <div className="rounded-2xl border border-ink-200/70 bg-white p-4 shadow-soft">
      <h3 className="mb-3 text-sm font-semibold text-ink-800">Upcoming deadlines</h3>
      {upcoming.length === 0 ? (
          ...
          {upcoming.map((t) => {
            ...
            return (
              ...
              >
                ...
                <span className="shrink-0 text-xs text-ink-400">{formatRelative(t.deadline)}</span>
              </button>
            )
          })}
        </div>
      ...
      <p className="mt-2 text-xs text-ink-400">Chip colors on the calendar match these statuses.</p>
    </div>
  )
}
